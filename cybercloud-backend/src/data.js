        ...
        if (rate != null) {
            ...
            for (let index = 0; index < terminals.length; index++) {
                ...
                if(terminals[index].user.id != "0"){
                    user = await User.findByIdAndUpdate(terminals[index].user.id);
                    money = user.money;
                }
                let time = 0;
                ...
                while (time > terminals[index].accumulated) {
                    if (terminals[index].user.id != "0") {
                        if (user.money >= rate.interval[terminals[index].rate_i].cost) {
                            await User.findByIdAndUpdate(terminals[index].user.id, {
                                money: (user.money - rate.interval[terminals[index].rate_i].cost)
                            });
                            money = (user.money - rate.interval[terminals[index].rate_i].cost);
                        }
                        else {
                    ...
                    terminals[index].accumulated = terminals[index].accumulated + rate.interval[terminals[index].rate_i].time;
                    terminals[index].price = terminals[index].price + rate.interval[terminals[index].rate_i].cost;
                    terminals[index].rate_i++;
                    if (terminals[index].rate_i == rate.interval.length) {
...
module.exports = { getTerminals, getTerminal };