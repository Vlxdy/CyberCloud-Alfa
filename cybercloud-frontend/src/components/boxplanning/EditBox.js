...
export default class EditBox extends Component {
    ...
    editBox = async () => {
        if (Date.parse(this.state.startDate) < Date.parse(this.state.endDate)) {
            await axios.put('http://' + global.ip + ':4000/api/box/'+this.props.box, {
    ...
    render() {
        return (
            <div className="card">
                ...
                <strong className="">Editar Caja #{this.props.box}</strong>
                </div>
                ...
                <div className="form-group">
                        <button className="btn btn-info m-2" onClick={() => this.editBox()}>Editar</button>
                        <button className="btn btn-danger" onClick={this.props.onCancelEdit}>Cancel</button>
                    </div>
                </div>
            </div>
        )
